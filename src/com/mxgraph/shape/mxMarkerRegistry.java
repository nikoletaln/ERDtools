...
public class mxMarkerRegistry
{
	...
	static
	{
		...
		registerMarker(mxConstants.ARROW_ARC, new mxIMarker()
		{
			public mxPoint paintMarker(mxGraphics2DCanvas canvas,
					mxCellState state, String type, mxPoint pe, double nx,
					double ny, double size, boolean source)
			{
				//double cx = pe.getX() ;
				//double cy = pe.getY()-20 ;
				//Shape shape = new Arc2D.Double(cx, cy , 20,20, 90, -180, Arc2D.OPEN);
				//canvas.getGraphics().draw(shape);
				
				double omega =  (270 + (Math.atan2(pe.getY(), pe.getX()) * 180 / Math.PI));
				canvas.getGraphics().draw(
                        new Arc2D.Double((int) Math.round(pe.getX()+ ny / 2 - nx),
                                (int) Math.round(pe.getY() - ny - nx / 2),20 ,20  ,
                                omega, 180, Arc2D.OPEN));
