...
public class ArcLineShape extends mxBasicShape {
	...
	private void translatePoint(List<mxPoint> points, int index, mxPoint offset)
	{
		if (offset != null)
		{
			mxPoint pt = (mxPoint) points.get(index).clone();
		 	pt.setX(pt.getX() + offset.getX()/10);
			pt.setY(pt.getY() + offset.getY()/10);
	...
	public mxPoint paintMarker(mxGraphics2DCanvas canvas, mxCellState state, boolean source)
	{
		...
		mxPoint pe = new mxPoint(centerX, centerY);
		...
		if (marker != null)
		{
			...
			if (offset != null)
			{
				...
				//
			//	offset = new mxPoint(40*strokeX , 40*strokeY );
			}
