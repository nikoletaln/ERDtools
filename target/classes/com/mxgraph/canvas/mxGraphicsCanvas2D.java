...
//creating a low dashed underline text	

		if ((state.fontStyle
		& mxConstants.FONT_UNDERLINE_LOW_DASHED) == mxConstants.FONT_UNDERLINE_LOW_DASHED)
		{		
			txtDecor += "underline; text-decoration-style: dashed";
		}
			...
			for (int i = 0; i < lines.length; i++)
			{
				...
				if (!lines[i].isEmpty())
				{
					...
					boolean isStrikethrough = (state.fontStyle
							& mxConstants.FONT_STRIKETHROUGH) == mxConstants.FONT_STRIKETHROUGH;
					//creating a low dashed underline text		
					boolean isUnderlineLowDashed = (state.fontStyle
							& mxConstants.FONT_UNDERLINE_LOW_DASHED) == mxConstants.FONT_UNDERLINE_LOW_DASHED;
					if (isUnderline || isStrikethrough || isUnderlineLowDashed)
					{
