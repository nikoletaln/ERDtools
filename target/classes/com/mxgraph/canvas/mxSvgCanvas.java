...
public class mxSvgCanvas extends mxBasicCanvas
{
			...
			Map<String, Object> style)
	{
		...
		else if (shape.contains(mxConstants.SHAPE_ELLIPSE)||shape.contains(mxConstants.DASHED)||shape.contains(mxConstants.PARTIAL)
				||shape.contains(mxConstants.PRIMARY)||shape.contains(mxConstants.UNIQUE)||shape.contains(mxConstants.D_CIRCLE)||shape.contains(mxConstants.O_CIRCLE)||shape.contains(mxConstants.U_CIRCLE))
		{
			...
			if (shape.contains(mxConstants.PRIMARY))
			{
				elem.setAttribute("font-weight", "bold");
				elem.setAttribute("text-decoration", "underline");
			}

			else if (shape.contains(mxConstants.UNIQUE))
			{
				elem.setAttribute("text-decoration", "underline");
			}

			// if (fontStyle==6)
			 else if (shape.contains(mxConstants.PARTIAL))
			 {
		 		Element line = document.createElement("line");
				line.setAttribute("x1",String.valueOf(x));
				line.setAttribute("y1",  String.valueOf(y + h / 2));
				line.setAttribute("x2", String.valueOf(x + w));
				line.setAttribute("y2",  String.valueOf(y + h / 2));
				line.setAttribute("stroke", "black");
				line.setAttribute("stroke-dasharray", "3,3");
				appendSvgElement(line);
				
		// //		elem.setAttribute("text-decoration", "underline; text-decoration-style: dashed;");
		// 	//	elem.setAttribute("text-decoration", "underline; text-decoration-style: dashed; color: black;");
		// 	//	elem.setAttribute("text-decoration-style", "dashed;");
			}
		}

		else if (shape.equals(mxConstants.SHAPE_DOUBLE_ELLIPSE)||shape.equals(mxConstants.UN_DL))
		{
		...
		else if (shape.equals(mxConstants.SHAPE_DOUBLE_RHOMBUS))
		{
   	 		elem = document.createElement("g");
    		background = document.createElement("path");

   			String outerRhombusD = "M " + (x + w / 2) + " " + y + " L " + (x + w) + " "
                            + (y + h / 2) + " L " + (x + w / 2) + " " + (y + h) + " L "
                            + x + " " + (y + h / 2) + " Z";

    		background.setAttribute("d", outerRhombusD);
    		elem.appendChild(background);
    		...
    		double innerRhombusWidth = w - 2 * inset;
   		   double innerRhombusHeight = h - 2 * inset;

    		double innerTopLeftX = x + inset;
    		double innerTopLeftY = y + inset;
    		double innerTopRightX = x + w - inset;
   			double innerTopRightY = y + inset;
    		double innerBottomLeftX = x + inset;
    		double innerBottomLeftY = y + h - inset;
    		double innerBottomRightX = x + w - inset;
    		double innerBottomRightY = y + h - inset;

    		String innerRhombusD = "M " + (innerTopLeftX + innerRhombusWidth / 2) + " " + innerTopLeftY +
                            " L " + innerTopRightX + " " + (innerTopRightY + innerRhombusHeight / 2) +
                            " L " + (innerTopLeftX + innerRhombusWidth / 2) + " " + innerBottomRightY +
                            " L " + innerBottomLeftX + " " + (innerBottomLeftY - innerRhombusHeight / 2) +
                            " Z";

    		foreground.setAttribute("d", innerRhombusD);
   			 elem.appendChild(foreground);
		}
		...
		else if (shape.equals(mxConstants.SHAPE_RECTANGLE))
		{
		...
		else if (shape.equals(mxConstants.SHAPE_DOUBLE_RECTANGLE))
{
    elem = document.createElement("g");

    Element innerBorder = document.createElement("rect");
    innerBorder.setAttribute("x", String.valueOf(x + 2));
    innerBorder.setAttribute("y", String.valueOf(y + 2));
    innerBorder.setAttribute("width", String.valueOf(w - 4));
    innerBorder.setAttribute("height", String.valueOf(h - 4));
    innerBorder.setAttribute("stroke-width", "1");
    innerBorder.setAttribute("stroke", "black");
	
    elem.appendChild(innerBorder);

    Element outerBorder = document.createElement("rect");
    outerBorder.setAttribute("x", String.valueOf(x));
    outerBorder.setAttribute("y", String.valueOf(y));
    outerBorder.setAttribute("width", String.valueOf(w));
    outerBorder.setAttribute("height", String.valueOf(h));
    outerBorder.setAttribute("stroke-width", "1");
    outerBorder.setAttribute("stroke", "black");
	outerBorder.setAttribute("fill", "none");
    elem.appendChild(outerBorder);
}
			...
			Map<String, Object> style)
	{
		...
		if (text != null && text.length() > 0)
		{
			...
			if ((fontStyle & mxConstants.FONT_UNDERLINE) == mxConstants.FONT_UNDERLINE  && fontStyle!=6)
		    {
				txtDecor += "underline";
				elem.setAttribute("text-decoration", txtDecor);
	    	elem.setAttribute("font-decoration",  txtDecor);
		    }
		   ...
		   if ((fontStyle & mxConstants.FONT_STRIKETHROUGH) == mxConstants.FONT_STRIKETHROUGH)
			...
			elem.setAttribute("fill", fontColor);
			  ...
			  if ((fontStyle & mxConstants.FONT_UNDERLINE_LOW_DASHED) == mxConstants.FONT_UNDERLINE_LOW_DASHED) {
				Element line = document.createElement("line");
				line.setAttribute("x1",String.valueOf(x));
				line.setAttribute("y1",  String.valueOf( y+ h ));
				line.setAttribute("x2", String.valueOf(x + w));
				line.setAttribute("y2",  String.valueOf( y+ h ));
				line.setAttribute("stroke", "black");
				line.setAttribute("stroke-dasharray", "3,3");
				appendSvgElement(line);
			  }
